...
Repeatable `--values` files provide platform image, runtime, and hardware settings. On a restricted network, `--oci-registry` selects one mirrored registry prefix for release images and CLI-managed charts; see [Restricted networks and mirrored sources](../docs/restricted-network.md).
