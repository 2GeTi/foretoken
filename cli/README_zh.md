...
重复使用 `--values` 可提供平台镜像、runtime 和硬件配置。editable 构建会自动比较受支持的匿名包源和构建镜像源；`--oci-registry` 用于为发布镜像和 CLI 管理的 Chart 显式指定统一 registry 前缀。Hugging Face 兼容模型 endpoint 继续由平台显式配置。详见[受限网络与镜像源](../docs/restricted-network_zh.md)。
