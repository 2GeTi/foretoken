...
选择功耗告警时，还需按显卡型号填写正数 `spec.observability.alerts.thresholds.nvidiaPowerWatts`，单位为瓦。只填写阈值不会启用规则。通知通过可选的 [Lark](integrations/lark/README_zh.md) 或 [Slack](integrations/slack/README_zh.md) 接收器单独配置；各集成指南说明了消息格式和接收目标的设置方法。
