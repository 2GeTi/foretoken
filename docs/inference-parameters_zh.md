...
开启 EP 后，attention 可以按 TP × DP 执行，路由专家则分布在对应的 EP 组中。共享专家不代表共享 attention 的 KV Cache：路由会选择模型执行组及其中的 DP rank，按各 rank 的前缀缓存观测做选择。某个 rank 的 KV 事件不可用或中断时，其缓存位置按未知处理，不计为命中，也不影响其他 rank 的观测。目前负载统计仍是整个执行组的汇总，不是逐 DP 队列统计。
