...
- **Time/** uses elapsed seconds for one-second completion-window counts, throughput, failure rate, p95 timings, and mean in-flight requests. The last window uses its actual duration.
- **Cumulative/** shows completed-request totals, success rate, mean timings, and throughput since the run began.
- **Requests/** uses request index in send order, starting at one, for individual timings, token counts, and success.
- **Replicas/** records controller-applied desired and Ready replicas for Kustomize runs, separately for each model service and scaling target. Raw samples are saved in `replica_observations.json`; collection ends with the workload, without waiting for subsequent scale-down.
