...
- **Time/** 按一秒完成窗口展示请求数、吞吐量、失败率、耗时 p95 和平均在途请求数；最后一个窗口使用实际时长。
- **Cumulative/** 展示已完成请求的累计数量、成功率、平均耗时和从运行开始计算的吞吐量。
- **Requests/** 按发送顺序展示逐请求耗时、token 数和成功状态，序号从 1 开始。
- **Replicas/** 为 Kustomize 评测记录控制器实际应用的期望副本和 Ready 副本，按模型服务和扩缩目标区分。原始记录保存在 `replica_observations.json`；采集随负载结束，不额外等待后续缩容。
