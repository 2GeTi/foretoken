...
Each trigger, decision, and adjustment stage owns a compiled descriptor beside its implementation; the stage descriptor list is the only registration point. Adding an algorithm requires its implementation and one descriptor in that stage. The top-level registry only validates those static descriptors and constructs the selected factory; it does not accept runtime registrations. Each implementation owns parameter decoding, defaults, validation, and execution; no algorithm enum or algorithm-specific CRD/compiler/controller mapping is maintained. `core.DecodeParameters` decodes explicitly selected fields without exposing implementation structs as configuration.
